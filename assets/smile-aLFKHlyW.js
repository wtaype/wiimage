import{o as Oa,b as u,h as na,q as V,i as W,e as g,w as F,s as m,f as k,$ as e,j as Pa,M as w,g as ha,a as ba,N as U,c as va,l as Y,k as ra,m as B,n as O,r as D,p as ka,t as ya}from"./widev-C2sx_5ry.js";import"./wiauth-DRAtMC-Z.js";Oa(k,async p=>{if(!p)return window.location.href="/";try{const d=u("wiSmile");if(d)return wa(d);const h=(await na(V(W(g,"smiles"),F("usuario","==",p.displayName)))).docs[0].data();m("wiSmile",h,450),wa(h)}catch(d){console.error(d)}});e(document).on("click",".bt_salir",async()=>{await Pa(k),window.location.href="/";try{localStorage.clear()}catch{Object.keys(localStorage).forEach(d=>localStorage.removeItem(d))}});e(document).on("click",".bt_cargar",()=>{const p=/^(im\d+|ki\d+|remote:im\d+|dirty:im\d+|dirty:ki\d+)$/;Object.keys(localStorage).filter(d=>p.test(d)).forEach(d=>localStorage.removeItem(d)),w("Actualizado"),setTimeout(()=>location.reload(),800)});function wa(p){console.log(p.nombre),w("Bienvenido "+p.nombre+"!"),e(".app").html(`
    <header class="hd">
